...
# Kept as a matching literal constant in build/gn_processor.py's
# TOPSRCDIR_PLACEHOLDER rather than imported, since importing this module
# from there would also execute `import buildconfig` above, which requires
# a configured objdir that may not exist at vendoring time.
...
def action(fh, script, target_dir, *args):
    ...
    args = list(args)
    topobjdir = Path(buildconfig.topobjdir)
    ...
    try:
        abs_target_dir = str(make_absolute(topobjdir, target_dir))
